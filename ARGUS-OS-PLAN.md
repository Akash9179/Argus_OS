...
- Color language, fixed: green healthy/friendly, amber attention, red act now, gray offline. No other status colors. Each color has a dark-mode and a day-mode value; the meaning never changes between them, only the value, so the four colors stay distinguishable against both surfaces.
- Design references: Anduril Lattice UI, ATAK. Fonts and brand per the ARGUS brand guidelines (JetBrains Mono, IBM Plex Sans, self-hosted, no CDN).
- **Themes (amended 2 August 2026, on founder instruction; supersedes the original "dark theme always").** Two themes ship: `dark`, the default, for control rooms and night work, where it preserves dark adaptation; and `day`, a high-brightness mode for working outdoors in sunlight, where a dark screen is unreadable. This is a legibility requirement, not a matter of taste, and it follows ATAK's precedent. A third setting, `auto`, switches between them at local sunrise and sunset. The setting is operator-accessible and instant, because daylight changes faster than an admin can be found. Applies to the OS-level UI and to every application.
...
*Built 3 August 2026. Two things about how it was built are worth recording rather than discovering later.*

*Speech goes through the gateway too, as `transcribe` and `speak` capabilities, which is stricter than this plan asked for. Section 8 only required intent handling to go through the gateway, but Whisper and Piper are models, and law 3 says no code outside the gateway may name one. Putting them behind gateway adapters means `voice/` names nothing and swapping either is an adapter change.*

*The air-gapped language path is **unproven**. The local adapter is written, registered, and selected by the `deployed` profile, and the profile's refusal of cloud adapters is enforced at the point of use and covered by tests. But no local language model has been installed, so that adapter has never answered a request. Speech is local and is exercised on every profile. Anyone judging the sovereignty story should read this paragraph before believing the rest.*
...
10. **Where autonomy level lives. DECIDED 3 August 2026, founder instruction: split in two and staged.** The question divides along what each side can know. Reacting to what a machine itself perceives can only live on the machine, or it dies with the link; reacting to what the fleet perceives can only live on the platform, because only the platform holds the fused picture. So: **fleet autonomy policy is per-asset state held and enforced by TRACK** (stage one, built now; no contract impact, because the mode never crosses the wire), and **machine-local standing orders travel as a normal task with an open task_type** (stage two, built with Stage 3B when machines have real perception worth standing orders over; a convention like decision 8, not a version bump, since `task_type` is open vocabulary and `TaskParameters.extras` is an open struct). The original context, kept for the record: Manual and Automatic began as a C2 browser-side policy at Stage 2, Stage 3A built the machine's autonomy core with no way to receive a standing instruction, Stage 4 built voice on top of the browser arrangement, and the C2 wording pass had to hedge every mode sentence because a browser cannot promise what a fleet does. The premise that the contract "has no field for it" was wrong in the way decision 8 already answered: the contract's open vocabulary is the field.
11. **Which cloud adapter, and whether one ships at all (raised and settled at Stage 4).** Section 7 asked for "one cloud adapter for bench development" without naming it. Anthropic was chosen on founder instruction, 3 August 2026. It is present in the `dev` and `demo` profiles and refused by `deployed`. Recorded because the choice was the founder's and because a second provider would be an adapter, not a change to any caller.
