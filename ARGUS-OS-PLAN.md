...
10. **Where autonomy level lives (raised at Stage 2, still open, now built around).** Manual and Automatic are a C2-side policy today. Autonomy properly belongs to the machine and travels over the contract, which has no field for it. Stage 3A built the autonomy core, which is where it belongs. Stage 4 has now been built on top of the current arrangement without settling this: voice orders carry no autonomy level and inherit whatever C2 is set to. That is one more thing to unpick when it is decided, and the cost of deferring it again keeps rising.
