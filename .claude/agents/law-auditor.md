...
Audit procedure:
1. Read ARCHITECTURE.md section 2 (the laws) and docs/architecture/graph/laws.yaml (where each is enforced). Component boundaries: ARCHITECTURE.md section 3 and PROJECT.md.
...
3. Check each law that applies. Priority checks, with concrete grep targets:
   ...
   - Evidence law (17): if the change alters a component's maturity, gaps, or blockers, docs/architecture/graph/components.yaml and the regenerated STATUS.md must be in the same change. A claim of "done", "working", or "hardware-tested" in docs without matching graph evidence is a violation.
